...
| Pedidos | `app/components/features/orders/OrdersModule.tsx` | deliveryOrders, products, stockMovements | FSM 6 estados; `/api/orders/public` é entrada anônima rate-limited; edição de itens/valores server-side (`editDeliveryOrderAdmin` via `PATCH /api/orders/[id]/edit`) — bloqueada fora de `recebido`, reconcilia estoque (restaura+deduz) dentro dele; `DeliveryType` = entrega\|retirada\|**mesa** (salão, com QR code opcional via `/p/{slug}?mesa=N` — ver `docs/paridade/M02_MESA_SALAO.md`); pedido com `tableSessionId` NÃO lança receita própria na entrega — sai pela Sale do PDV no fechamento da comanda (ver Mesas) |
| Mesas | `app/components/features/mesas/MesasModule.tsx` | **tableSessions**, deliveryOrders | Comanda de mesa — FSM `aberta→fechada→paga\|cancelada` (`lib/contracts/fsm/tableSession.ts`). Agrega vários deliveryOrders (`deliveryType='mesa'`) por mesa; "fechar conta" congela o subtotal; "enviar pro PDV" consolida os itens no checkout (Sale única, receita+NFC-e 1x). `settleTableSessionAdmin` marca cada pedido `entregue` com `settledViaSaleId` — SEM receita própria (guarda anti-receita-dobrada). WRITE é Admin-SDK-only (`/api/table-sessions/*`); client só lê. Ver `docs/paridade/M02_MESAS_COMANDA.md` |
...
| Agenda | `app/components/features/agenda/AgendaModule.tsx` | appointments, services, clients, transactions | conflict-check inline; conclusão/reversão (commission + loyalty + métricas + baixa de insumo) via evento `appointment.completed`/`canceled` → handler server-side (hardening odontologia); GCal push + cron reminders continuam inline; botão manual "Emitir NFSe" no atendimento concluído (`EmitirNotaDialog` reaproveitado, writeback `fiscalDocumentId`/`fiscalAccessKey` — ver `docs/agenda/AGENDA_NFSE_MANUAL.md`) |
