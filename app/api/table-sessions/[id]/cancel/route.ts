...
import { CancelTableSessionBodySchema } from '@/lib/contracts/api/tableSession';
import { cancelTableSessionAdmin } from '@/lib/services/table-session-admin';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = CancelTableSessionBodySchema.safeParse(raw);
  ...
  if ((ROLE_HIERARCHY[auth.role as UserRole] ?? 0) < ROLE_HIERARCHY.operator) {
    return jsonError('Sem permissão para cancelar mesas.', 403);
  }

  try {
    const session = await cancelTableSessionAdmin({
      db: adminDb,
      sessionId: id,
